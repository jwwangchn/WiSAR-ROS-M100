...
#define USE_SEARCHING_GPS_FROM_MOBILE
...
int main ( int argc, char **argv )
{
    ...
    double goalGPSPointLatitude = 30.0;
    double goalGPSPointLongitude = 114.0;
    ...
    node_priv.param<double> ( "initDescedingHeightSearch",initial_descending_height_at_search,30.0 );
    node_priv.param<double> ("goalGPSPointLatitude",goalGPSPointLatitude,30.0);
    node_priv.param<double> ("goalGPSPointLongitude",goalGPSPointLongitude,114.0);
...
//Add for FLIR VUE to record video.
    //ros::Publisher flir_record_pub = nh.advertise<std_msgs::Bool>("record_infrared",100);
    ...
    drone->gimbal_angle_control ( 0,-900,1800,20 ); //Head down at the beginning.
    ...
    waypoint0.latitude = goalGPSPointLatitude;//22.5428000;   //43.2307347;
    waypoint0.longitude = goalGPSPointLongitude;//113.9589000;  //-75.41944035;
  ...
  //  start_yaw = drone->yaw_from_drone;
    ...
    while ( ros::ok() )
    {
        ...
        case 'q':
        {
...
#ifdef USE_SEARCHING_GPS_FROM_MOBILE
		      waypoint0.latitude = drone->targetSearchingLATT;//22.5428000;   //43.2307347;
                      waypoint0.longitude = drone->targetSearchingLONG;//113.9589000;  //-75.41944035;
		    ...
		    /*
                    sleep(20);
                     writeF<<"waypoint. state=0, navigation finished"<<ob_distance[0]<<" "<<(unsigned char)(drone->waypoint_mission_push_info.current_status)<<endl;
                    while ( drone->transparentdata.data.at ( 0 ) != 'a'&& drone->waypoint_mission_push_info.current_status==0xEC)// drone->mission_waypoint_get_speed() >0.10 )
		    {		
		      ROS_INFO("Here!!!!!!!!!!");
                      ros::spinOnce();
		     }
		    
                    writeF<<"waypoint. state=0, navigation finished, descending"<<ob_distance[0]<<" "<<drone->waypoint_mission_push_info.current_status<<endl;
                    */
		    			
		    for(int i = 0;i < loopTimes;i++)
		        {
		            drone->attitude_control(0x9B, 1, 0, initial_descending_height_at_search, 0);
				ros::spinOnce();
		            usleep(20000);
		        }
		    state_in_mission = 1;
		    //drone->gimbal_angle_control ( 0,-900,0,10,1 );
                    break;
                    /***********************ACTION OF MISSIONS*******************************************/
                case 1://
		    ...
		    if(++zigzagCnt<10)
		    {
		      ...
		      for(int i = 0;i < loopTimes;i++)
		        {
		            drone->attitude_control(0x9B, 1, 0, flying_height_control_searching, 0);
				ros::spinOnce();
		            usleep(20000);
		        }
		        for(int i = 0;i < loopTimes;i++)
		        {
		            drone->attitude_control(0x9B, -1, 0.3, flying_height_control_searching, 0);
				ros::spinOnce();
		            usleep(20000);
		        }
		    }
			...
			for(int i = 0;i < loopTimes;i++)
		        ...
		        for(int i = 0;i < loopTimes;i++)
		        {
		            drone->attitude_control(0x9B, 0, 1, flying_height_control_searching, 0);
				ros::spinOnce();
		            usleep(20000);
		        }
		        for(int i = 0;i < loopTimes;i++)
	...
	//{
	 // std_msgs::Bool startRecord;
	  ...
	  //flir_record_pub.publish(startRecord);
	  //break;
	//}
	//case 't':
	//{
	  ...
	  //Record.data = false;
	  //flir_record_pub.publish(Record);
	  //break;
	//}
