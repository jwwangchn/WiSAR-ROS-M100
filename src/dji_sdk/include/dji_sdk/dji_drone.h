...
public:
  ...
  //Add this for reading searchingStartGPS location
  
  double targetSearchingLONG;
  double targetSearchingLATT;
...
//Add a callback for apriltag_detetion
  ...
  void data_received_from_mobile_callback ( const dji_sdk::TransparentTransmissionData transparentdata )
  {
    ...
    if(transparentdata.data.size()<2)
    {
      switch(transparentdata.data.at ( 0 ))
      {
	case 'q':
...
*/      
          ...
          //Reading startSearchingGPS location
    else //if(transparentdata.data.size()>=19)
    {
      vector <uint8_t> GPS(transparentdata.data.begin(),transparentdata.data.end());
      sscanf((char*)(&GPS[0]),"LONG%lfLATT%lf",&(this->targetSearchingLONG),&(this->targetSearchingLATT));

      string info(GPS.begin(),GPS.end());
      cout<<info<<endl;
      ROS_INFO("GPS:LONG->%f LATT->%f",this->targetSearchingLONG,this->targetSearchingLATT);
      if(!send_data_to_mobile_devide(GPS,GPS.size()))
      ROS_INFO("Send data to mobile. Failed.");
    } 
      
      ROS_INFO("TR LENGTH:%d",transparentdata.data.size());
  }
...
public:
  DJIDrone ( ros::NodeHandle& nh ) :
    ...
    flight_norm ( 6.0 ), //
    flight_height(6.0),
    targetSearchingLONG(0.000000),
    targetSearchingLATT(0.000000)
  {
/// binding service client to specific service;
    ...
    flir_record_pub = nh.advertise<std_msgs::Bool>("record_infrared",100);
